...
class MICCAI(Dataset):
    ...
    def __getitem__(self, index):
        # Open image
        default_patient_images = self.patients[list(self.patients.keys())[index]]
        sample_patient_images = []
        # Sample options.stack_size images from this patient
        if len(default_patient_images) < options.stack_size:
            for i in range(options.stack_size):
                # Sampling with replacement
                original = random.sample(default_patient_images, 1)
                ...
                sample_patient_images.append(img)
        else:
            for i in range(options.stack_size):
                # Sampling without replacement
                original = random.sample(default_patient_images, 1)
                default_patient_images.remove(original[0])
                file_name = original[0]
        ...
        target_label = 'AMR' if default_patient_images[0][0] == 'A' else 'Non-AMR'
